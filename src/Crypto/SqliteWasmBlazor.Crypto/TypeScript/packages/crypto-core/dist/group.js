 ...
 */
export async function createGroupKeys(adminPrivateKey, adminPublicKey, memberPublicKeys, groupContext) {
 ...
 */
export async function addGroupMembers(adminPrivateKey, adminPublicKey, adminWrappedCek, newMemberPublicKeys, groupContext) {
 ...
 */
export async function rotateGroupKey(adminPrivateKey, adminPublicKey, remainingMemberPublicKeys, groupContext) {
 ...
 */
export async function transferGroupAdmin(oldAdminPrivateKey, oldAdminPublicKey, oldAdminWrappedCek, newAdminPrivateKey, newAdminPublicKey, memberPublicKeys, groupContext, keyVersion) {
 ...
 */
export async function encryptForGroup(senderX25519PrivateKey, senderEd25519PrivateKey, senderEd25519PublicKey, adminPublicKey, senderWrappedCek, plaintext, groupContext, keyVersion) {
    // Unwrap sender's CEK
 ...
 */
export async function decryptFromGroup(recipientX25519PrivateKey, adminPublicKey, recipientWrappedCek, message) {
    // Layer 2: Verify envelope signature
    const envelopePayload = buildCanonicalEnvelope(message.groupContext, message.keyVersion, message.senderPublicKey, message.encrypted);
    const envelopeBytes = new TextEncoder().encode(envelopePayload);
 ...
 */
async function unwrapMemberCek(memberPrivateKey, adminPublicKey, wrappedCek, groupContext) {
 ...
 */
function buildCanonicalEnvelope(groupContext, keyVersion, senderPublicKey, encrypted) {
 ...
 */
function parseVersionFromContext(groupContext) {
