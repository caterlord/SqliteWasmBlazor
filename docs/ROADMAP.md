...
_Single source of truth for "where are we." Last updated 2026-04-29 against branch `crypto-sync` HEAD `4f1ae1b`._
...
5. **IN FLIGHT** — DI wiring + scenario-completeness sweep (test seeds throughout). Split into 5a (DI extensions, ✅ DONE `4f1ae1b`) + 5b (live-relay scenario sweep, NEXT).
   ...
   - 5b ⏭ **NEXT** — extend `HttpSyncTransportLiveRelayTests` with: three-actor scenario (admin + user1 + user2 each posts/receives), invitation-roundtrip, grace-window expiry (`READ_GRACE_SECONDS=2`), body-cap enforcement (`>max_body_bytes` → 413), sender-sig timestamp window (`X-Timestamp = now - 600` → 401). Replay defense + non-admin push are already covered.
