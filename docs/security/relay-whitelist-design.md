...
The whitelist-broadcast model the DeltaRelay PHP service implements:
authenticated POST (whitelisted Ed25519 pubkeys only), unbounded
broadcast read with metadata stripped on the relay side, admin-only
purge with pinned-seed initialization. This document is the design
rationale paired with [`../crypto-vfs.md`](../crypto-vfs.md) for the
at-rest layer and [`threat-model.md`](threat-model.md) for the overall
attacker model.
...
// Rate limiting lives at the webserver layer (nginx limit_req_zone /
// Apache mod_ratelimit) — REQUIRED in production. The relay PHP layer
// does not enforce per-IP or per-pubkey limits.
```
