 ...
 */
export async function encryptPushPayload(plaintext, subscriptionKeys) {
    ...
    const subscriberKey = await crypto.subtle.importKey('raw', toBuffer(subscriptionKeys.p256dh), { name: 'ECDH', namedCurve: 'P-256' }, false, []);
    // ECDH key agreement
    ...
    const { contentKey, nonce } = await deriveKeyAndNonce(sharedSecret, salt, subscriptionKeys.auth, localPublicKey, subscriptionKeys.p256dh);
    ...
    const ciphertext = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv: toBuffer(nonce), tagLength: 128 }, contentKey, toBuffer(padded)));
 ...
 */
export async function sendPushNotification(endpoint, subscriptionKeys, payload, vapidPrivateKey, vapidPublicKey, subject, proxyUrl, apiKey, ttl = 86400) {
    ...
    const authorization = await createVapidAuthHeader(audience, subject, vapidPrivateKey, vapidPublicKey);
 ...
 */
async function deriveKeyAndNonce(sharedSecret, salt, authSecret, senderPublicKey, subscriberPublicKey) {
    const encoder = new TextEncoder();
    // Step 1: Derive IKM from ECDH shared secret + auth secret
    // info = "WebPush: info" || 0x00 || subscriber_pub || sender_pub
    const infoIkm = concatBytes(encoder.encode('WebPush: info\0'), subscriberPublicKey, senderPublicKey);
    ...
    const contentKey = await crypto.subtle.importKey('raw', toBuffer(cekBits), { name: 'AES-GCM' }, false, ['encrypt']);
 ...
 */
async function hkdfDeriveBits(salt, ikm, info, bits) {
    const baseKey = await crypto.subtle.importKey('raw', toBuffer(ikm), { name: 'HKDF' }, false, ['deriveBits']);
